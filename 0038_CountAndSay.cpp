...
n other words from the previous member read off the digits, counting the number of digits in groups of the same digit.
...
public:
    string countAndSay(int n){
        ...
        {
            int c=1;
            while(sA[j]==sA[j+1])  //null char there 
            {
                c++;
                j++;
            }
            ans+=to_string(c)+sA[j];
        }
        return ans;
    }
};
